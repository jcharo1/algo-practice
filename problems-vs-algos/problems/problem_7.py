...
class Router:
    def __init__(self,  handler=None, not_found=None):
        ...
        self.handler = handler
        self.not_found = not_found if not_found != "" or not_found is not None else "404 Page Not Found"
    def add_handler(self, path, handler):
        ...
        current_node.insert(path_list,handler)
    ...
    def lookup(self, path):
        ...
        if path == "":
            return "Please insert a / for root handler"
        if path == '/':
            return "Root Handler"
        current_node = self.root.root
        path_list = self.split_path(path)
...
print(router.lookup("")) # should print 'Please insert a / for root handler'
